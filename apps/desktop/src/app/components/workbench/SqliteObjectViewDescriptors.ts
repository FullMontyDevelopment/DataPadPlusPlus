...
const DESCRIPTORS: Record<string, SqliteObjectViewDescriptor> = {
  ...
  dependencies: descriptor('dependencies', 'Open Dependencies', 'SQLite Dependencies', 'Review table and view names detected from stored SQL definitions.', 'No dependencies were detected', 'SQLite does not keep a dependency catalog, so DataPad++ derives this from object SQL.'),
  ...
  maintenance: descriptor('maintenance', 'Open Maintenance', 'SQLite Maintenance', 'Run integrity checks, review journaling/storage posture, and prepare analyze, optimize, vacuum, or backup workflows.', 'No maintenance metadata is loaded', 'Refresh maintenance to collect SQLite health signals.'),
