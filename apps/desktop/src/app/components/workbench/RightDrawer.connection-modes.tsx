...
import { SearchConnectionFields } from './RightDrawer.search-connection-fields'
...
import { TimeSeriesConnectionFields } from './RightDrawer.timeseries-connection-fields'
import { WarehouseConnectionFields } from './RightDrawer.warehouse-connection-fields'
...
export function ConnectionModeFields({
  ...
  if (activeMode === 'cloud-iam' || activeMode === 'cloud-sdk') {
    if (connectionDraft.engine === 'dynamodb') {
      return (
        <DynamoDbConnectionFields
          connectionDraft={connectionDraft}
          mode={activeMode}
          secretDraft={secretDraft}
          onSecretDraftChange={onSecretDraftChange}
    ...
    if (connectionDraft.engine === 'cosmosdb') {
      return (
        <CosmosDbConnectionFields
          connectionDraft={connectionDraft}
          mode={activeMode}
          secretDraft={secretDraft}
          onSecretDraftChange={onSecretDraftChange}
    ...
    if (isSearchConnectionEngine(connectionDraft.engine)) {
      return (
    ...
    if (isWarehouseConnectionEngine(connectionDraft.engine)) {
      return (
        <WarehouseConnectionFields
          connectionDraft={connectionDraft}
          secretDraft={secretDraft}
...
function NativeConnectionFields({
  ...
  return (
    <>
      ...
      {connectionDraft.engine === 'cassandra' ? (
        <CassandraAdvancedFields
          connectionDraft={connectionDraft}
          onUpdateConnectionDraft={onUpdateConnectionDraft}
        />
      ) : null}

      {isSearchConnectionEngine(connectionDraft.engine) ? (
        <SearchConnectionFields
          connectionDraft={connectionDraft}
          mode="native"
          secretDraft={secretDraft}
      ...
      {isTimeSeriesConnectionEngine(connectionDraft.engine) ? (
        <TimeSeriesConnectionFields
          connectionDraft={connectionDraft}
          secretDraft={secretDraft}
          onSecretDraftChange={onSecretDraftChange}
          onUpdateConnectionDraft={onUpdateConnectionDraft}
      ...
      {isGraphConnectionEngine(connectionDraft.engine) ? (
        <GraphConnectionFields
          connectionDraft={connectionDraft}
          secretDraft={secretDraft}
          onSecretDraftChange={onSecretDraftChange}
          onUpdateConnectionDraft={onUpdateConnectionDraft}
      ...
      {isWarehouseConnectionEngine(connectionDraft.engine) ? (
        <WarehouseConnectionFields
          connectionDraft={connectionDraft}
          secretDraft={secretDraft}
          onSecretDraftChange={onSecretDraftChange}
...
function isTimeSeriesConnectionEngine(engine: ConnectionProfile['engine']) {
  return engine === 'prometheus' || engine === 'influxdb' || engine === 'opentsdb'
}

function isGraphConnectionEngine(engine: ConnectionProfile['engine']) {
  return engine === 'neo4j' || engine === 'arango' || engine === 'janusgraph' || engine === 'neptune'
}

function isWarehouseConnectionEngine(engine: ConnectionProfile['engine']) {
  return engine === 'snowflake' || engine === 'bigquery' || engine === 'clickhouse' || engine === 'duckdb'
}
