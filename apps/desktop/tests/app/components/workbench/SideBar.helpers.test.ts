...
describe('sidebar connection tree helpers', () => {
  ...
  it('builds connection object nodes from live explorer metadata', () => {
    ...
    expect(products?.actions).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          objectViewNodeId: 'collection-admin:rename-collection:catalog:products',
        }),
        expect.objectContaining({
          id: 'modify-collection',
          command: 'open-object-view',
          objectViewKind: 'collection',
          objectViewNodeId: 'collection-admin:modify-collection:catalog:products',
        }),
        expect.objectContaining({
          id: 'convert-to-capped',
          command: 'open-object-view',
          objectViewKind: 'collection',
          objectViewNodeId: 'collection-admin:convert-to-capped:catalog:products',
        }),
        expect.objectContaining({
          id: 'clone-as-capped',
          command: 'open-object-view',
          objectViewKind: 'collection',
          objectViewNodeId: 'collection-admin:clone-as-capped:catalog:products',
        }),
        expect.objectContaining({
          id: 'compact-collection',
          command: 'open-object-view',
          objectViewKind: 'collection',
          objectViewNodeId: 'collection-admin:compact-collection:catalog:products',
        }),
        expect.objectContaining({
          id: 'validate-collection',
          command: 'open-object-view',
          objectViewKind: 'collection',
          objectViewNodeId: 'collection-admin:validate-collection:catalog:products',
        }),
        expect.objectContaining({
          id: 'drop-collection',
          command: 'open-object-view',
          objectViewKind: 'collection',
          objectViewNodeId: 'collection-admin:drop-collection:catalog:products',
          separatorBefore: true,
        }),
      ]),
    )
    ...
    expect(connectionTreeNodeForAction(products!, renameAction!)).toMatchObject({
      id: 'collection-admin:rename-collection:catalog:products',
