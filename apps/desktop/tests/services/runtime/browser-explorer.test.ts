...
import { createExplorerNodes, inspectExplorerNodeLocally } from '../../../src/services/runtime/browser-explorer'
...
describe('browser explorer runtime', () => {
  ...
  it('does not invent sample nodes for unimplemented preview families', () => {
    const documentNodes = createExplorerNodes(genericPreviewConnection('experimental-document' as ConnectionProfile['engine'], 'document'))
    const keyValueNodes = createExplorerNodes(genericPreviewConnection('experimental-keyvalue' as ConnectionProfile['engine'], 'keyvalue'))
    const sqlNodes = createExplorerNodes(genericPreviewConnection('experimental-sql' as ConnectionProfile['engine'], 'sql'))

    expect(documentNodes).toEqual([])
    expect(keyValueNodes).toEqual([])
    expect(sqlNodes).toEqual([])
    expect(createExplorerNodes(genericPreviewConnection('experimental-sql' as ConnectionProfile['engine'], 'sql'), 'schema:public')).toEqual([])
