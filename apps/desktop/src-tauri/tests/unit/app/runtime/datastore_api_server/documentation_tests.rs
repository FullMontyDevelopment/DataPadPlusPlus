...
#[test]
fn datastore_provider_registry_has_exactly_one_provider_per_engine() {
    let engines = [
    ...
    for engine in engines {
        assert_eq!(
            provider_registration_count(engine),
            1,
            "{engine} should have exactly one API datastore provider"
        );
    }
}

#[test]
fn datastore_providers_own_resource_kinds_languages_and_requests() {
    assert_eq!(supported_resource_kinds("keyvalue", "redis"), vec!["key"]);
    assert_eq!(supported_resource_kinds("document", "mongodb"), vec!["collection"]);
    assert_eq!(supported_resource_kinds("sql", "postgresql"), vec!["table"]);
    assert_eq!(language_for_provider("dynamodb"), "json");
    assert_eq!(schema_hint_for_provider("elasticsearch"), "search");
    ...
    assert_eq!(
        read_query_for("keyvalue", "redis", &resource, 100, None).unwrap(),
        "GET customer:1"
    );
    assert_eq!(
        edit_kind_for("keyvalue", "redis", "key", "PATCH").unwrap(),
        "set-key-value"
    );
}
