...
#[test]
fn security_provider_registry_has_exactly_one_provider_per_engine() {
    let engines = [
    ...
    for engine in engines {
        assert_eq!(
            security_provider_registration_count(engine),
            1,
            "{engine} should have exactly one security provider"
        );
    }
}
