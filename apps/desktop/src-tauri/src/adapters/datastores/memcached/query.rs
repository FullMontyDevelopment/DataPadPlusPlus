...
use super::protocol::{memcached_request, memcached_request_bytes};
use super::query_results::{memcached_get_result_bytes, memcached_stats_result};
...
pub(super) async fn execute_memcached_query(
    ...
    if matches!(command.as_str(), "get" | "gets") && parts.len() > 1 {
        let raw = memcached_request_bytes(connection, &request_text).await?;
        let (payloads, summary) = memcached_get_result_bytes(&raw, &parts[1..])?;
