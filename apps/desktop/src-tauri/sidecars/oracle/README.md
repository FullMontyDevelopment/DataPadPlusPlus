...
This self-contained .NET 10 sidecar provides the built-in Oracle 19c+ runtime. It uses Oracle's fully managed ODP.NET Core provider and communicates with the Rust desktop process through newline-delimited JSON on stdin/stdout. It never opens a local listening port.
