...
> = {
  ...
  elasticsearch: {
    ...
    targetPhase: 3,
    summary:
      'Native-complete for the scoped Elasticsearch plain-HTTP search workflow: Elasticsearch has typed connection-flow parity with explicit disabled reasons for HTTPS/cloud/token/TLS/SigV4 profiles, pinned native tree parity, aggregation-aware Query DSL helpers, bounded live search, search hits/aggregation/profile renderers, normalized profile stages, native cluster/index/mapping/security/diagnostics/slow-log/allocation views, explicit-id document edits with before/after `_doc` evidence pinned by desktop live-scope tests, native repository snapshot backup with fail-on-conflict naming, isolated create-new index restore with rollback, preview-first execution gates for other index/template/pipeline/lifecycle/security/bulk/admin plans, and optional search fixture validator evidence for seeded index volume, mappings, aggregation/profile payloads, document evidence, slow-log/allocation diagnostics, bounded `_search` export plus `_bulk` import primitives, and snapshot type fidelity. Production cloud auth, ES|QL, broader live admin execution, and repository registration remain optional extensions outside the scoped claim.',
  ...
  opensearch: {
    ...
    targetPhase: 3,
    summary:
      'Native-complete for the scoped OpenSearch plain-HTTP search workflow: OpenSearch has typed connection-flow parity with explicit disabled reasons for HTTPS/managed/token/TLS/SigV4 profiles, pinned native tree parity, aggregation-aware Query DSL helpers, bounded live search, search hits/aggregation/profile renderers, normalized profile stages, native cluster/index/mapping/security/diagnostics/slow-log/allocation views, ISM/security-aware request routing, typed managed/SigV4 options, explicit-id document edits with before/after `_doc` evidence pinned by desktop live-scope tests, native repository snapshot backup with fail-on-conflict naming, isolated create-new index restore with rollback, preview-first execution gates for other index/template/pipeline/lifecycle/security/bulk/admin plans, and optional search fixture validator evidence for seeded OpenSearch volume, mappings, aggregation/profile payloads, document evidence, slow-log/allocation diagnostics, bounded `_search` export plus `_bulk` import primitives, snapshot type fidelity, and OpenSearch SQL, ISM, security, and Performance Analyzer boundary evidence. Managed SigV4/IAM runtime execution, OpenSearch SQL plugin execution, Performance Analyzer dashboards, broader live admin execution, and repository registration remain optional extensions outside the scoped claim.',
    profile: SEARCH_PROFILE,
  },
  dynamodb: {
    readiness: 'native',
