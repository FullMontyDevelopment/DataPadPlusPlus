...
| Expanding surfaces | Graph stores, warehouses, time-series, metrics, and more |
...
Oracle Database 19c+ is supported through a bundled managed runtime. Ordinary connection profiles provide live connection testing, permission-aware object browsing, SQL and PL/SQL execution, DBMS output, bounded results, and live metadata-backed IntelliSense without requiring Oracle Client, SQLPlus, Docker, or a separate .NET installation. Service name, SID, Easy Connect, TNS, TCPS/cloud wallet, proxy user, and supported administrative roles are available; SQLPlus remains an explicit legacy fallback. See [Oracle Support](docs/oracle.md) for connection, transaction, and safety boundaries.
