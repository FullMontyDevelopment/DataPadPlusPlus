...
All declared engines are now accepted for the contract-complete native UX gate, and MongoDB, PostgreSQL, CockroachDB, SQL Server, MySQL, MariaDB, Redis, SQLite, and Valkey are native-complete for scoped claims. MongoDB covers the scoped document workflow with live guarded insert/replace/delete and field edits, guarded desktop collection import/export, adapter-driven fixture evidence, and optional permission/management fixture validation. PostgreSQL covers the scoped SQL workflow with typed native connection/profile options, live SQL, primary-key row edits with before/after evidence, PostgreSQL-aware IntelliSense, extension/security/diagnostic object views, guarded routine/session/role/extension operations, rendered `EXPLAIN ANALYZE` dashboards, guarded desktop CSV/JSON/NDJSON table import/export, bounded JSON/SQL logical backup packages, and optional fixture validation; full `pg_dump`/`pg_restore` execution remains outside the scoped claim unless promoted later. CockroachDB covers the scoped distributed SQL workflow with typed deployment/cloud/region/version profile metadata, auth/TLS disabled reasons, capability-gated jobs/ranges/regions/diagnostics/security/zone surfaces, Cockroach-owned trees, native posture panels, CockroachDB query helpers and IntelliSense snippets, and guarded jobs/ranges/regions/sessions/contention/role/default-privilege/zone/import/export/backup/restore planners; live cloud capability probing, EXPLAIN ANALYZE DEBUG dashboards, job-control execution, zone changes, and live destructive/data-movement execution remain optional extensions outside the scoped claim. SQL Server covers the scoped SQL Server/Azure SQL workflow with typed connection options and mode-specific disabled reasons, live TDS SQL, primary-key row edits, shared SHOWPLAN_TEXT and XML Showplan rendering, runtime DMV profile payloads, Query Store workload panels, Extended Events, Agent, security/storage catalog panels, guarded statistics/index/Query Store previews, guarded desktop CSV/JSON/NDJSON table import/export, bounded JSON/SQL logical backup package execution, and restore-package validation; native `.bak` BACKUP/RESTORE, bcp/sqlcmd bulk workflows, identity insert, and broader live maintenance/admin execution stay outside the scoped claim. MySQL covers the scoped MySQL workflow with typed native connection/profile options, MySQL-native query helpers and IntelliSense, Workbench-style trees, native storage/index/security/session/status/performance_schema/optimizer/InnoDB/replication object views, detailed statement digest, table/index I/O, metadata-lock, optimizer-trace, and status-counter sections, live status/processlist/performance_schema/optimizer diagnostics, structured guarded maintenance/routine/event/security/user previews, guarded desktop CSV/JSON/NDJSON table import/export, bounded JSON/SQL logical backup packages, and restore-package validation; cleartext/IAM auth, selected live admin execution, `LOAD DATA INFILE`, mysqlpump/mysqldump parity, richer grant editing, replication-channel depth, and full restore execution stay outside the scoped claim. MariaDB covers the scoped MariaDB workflow with the MySQL-family live SQL/edit base, typed MariaDB connection/profile metadata, MariaDB-aware Workbench-style trees, explicit `EXPLAIN FORMAT=JSON` and guarded `ANALYZE FORMAT=JSON` previews, native role-mapping/server-variable/storage-engine/ANALYZE object views, MariaDB status/version/storage-engine and role helpers, Aria metrics, role-mapping security previews, preview-first routine/event/admin/user workflows, guarded desktop CSV/JSON/NDJSON table import/export, bounded JSON/SQL logical backup packages, and restore-package validation; selected live admin execution, `LOAD DATA INFILE`, mariadb-dump/mysql dump parity, richer role/grant editing, and full restore execution stay outside the scoped claim. Redis covers the scoped Redis and Redis Stack key workflow with native key browser/console surfaces, command-aware IntelliSense, guarded core/member edits, stream entry edits, RedisJSON path edits, RedisTimeSeries sample edits, vector-set member/attribute edits, guarded JSON/NDJSON key import/export, and Redis DUMP/RESTORE module snapshots; vector-set live fixture evidence remains an optional `--require-vector` check for Redis Stack images that expose `VADD`, and cluster/Sentinel evidence stays outside the scoped claim. SQLite covers the scoped local-file SQL workflow with local file connection flow, native tree/query/IntelliSense/object-view surfaces, live primary-key row edits, rendered explain/bytecode payloads, guarded maintenance previews, and guarded desktop `VACUUM INTO` backup plus CSV/JSON/NDJSON table export/import workflows; restore execution, encrypted-provider builds, and deeper trigger/index authoring stay outside the scoped claim. Valkey covers the scoped core single-node Redis-compatible workflow, with Valkey-specific tree/preview/menu wording while Redis Stack/vector-only file formats stay hidden unless compatible live metadata proves support. TimescaleDB is the active core SQL native-completion target; its PostgreSQL-wire base, typed deployment/profile metadata, capability-hiding, primary-key SQL row edits, hypertable/chunk/compression/retention/continuous-aggregate/job tree, profile posture panels, and guarded policy/import/export/backup previews are usable while rendered time-bucket dashboards, live metadata depth, import/export polish, optional fixture evidence, and guarded live policy execution remain open. Elasticsearch/OpenSearch explicit-id document edits and DynamoDB complete-key item edits are pinned as live-capable desktop scopes. Secondary datastore families now have pinned native connection-flow, object-tree, object-view, guarded-operation, diagnostics/performance, and import/export parity across shared, browser, and Rust contracts. DuckDB, ClickHouse, Snowflake, and BigQuery expose pinned SQL SELECT builders, and Prometheus/InfluxDB/OpenTSDB plus Neo4j/ArangoDB/JanusGraph/Neptune expose pinned query-builder descriptors while keeping mutation paths preview-first. Oracle, Cassandra, local/document/cache, time-series, graph, and warehouse admin/import/diagnostics flows keep explicit contract-only residual risks for optional live validation.

TimescaleDB now also has typed deployment/profile metadata, capability-hiding for restricted catalog surfaces, rendered profile posture, connection-test extension warnings, and profile-specific policy disabled reasons. The active Timescale work has moved to richer dashboards, live metadata depth, import/export polish, optional fixture evidence, and guarded live policy execution.
