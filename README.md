...
- LiteDB local files, collections, indexes, file storage, storage health, local-file read/write open preflight, encryption/lock-boundary metadata, configured sidecar read-dispatch contracts with local sidecar-process evidence, sidecar-only document CRUD plans, optional .NET engine read/edit validation, checkpoint/compact, backup/export, and guarded local management previews
