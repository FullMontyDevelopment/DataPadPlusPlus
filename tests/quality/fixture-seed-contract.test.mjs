...
test('DuckDB optional fixtures cover local read, profile, file workflow, and guard evidence', async () => {
  ...
  assert.match(integrationSource, /createdNewDatabase/)
  assert.match(integrationSource, /restoredTableCount/)
  assert.match(integrationSource, /restore_conflict/)
  ...
  assert.match(completenessSource, /live table import\/export[\s\S]*live EXPORT DATABASE backup folders[\s\S]*live IMPORT DATABASE restore into new isolated files/)
  ...
  assert.match(completenessSource, /embedded integration evidence for restore round trips, conflict rejection, corrupt-package cleanup, and post-restore catalog validation/)
})
...
test('LiteDB optional fixtures cover local-file preflight and sidecar boundary evidence', async () => {
  ...
  assert.match(dotnetValidatorSource, /operation: 'BackupDatabase'/)
  assert.match(dotnetValidatorSource, /operation: 'RestoreDatabase'/)
  assert.match(dotnetValidatorSource, /post-backup-only/)
  assert.match(dotnetValidatorSource, /litedb-backup-target-exists/)
  assert.match(dotnetValidatorSource, /litedb-restore-target-exists/)
  ...
  assert.match(importExportSource, /litedb\.data\.backup-restore/)
  assert.match(importExportSource, /BackupDatabase/)
  assert.match(importExportSource, /RestoreDatabase/)
  assert.match(importExportSource, /exactFileCopy/)
  ...
  assert.match(contractSource, /litedb\.data\.backup-restore/)
  ...
  assert.match(sidecarProgramSource, /BackupDatabase/)
  assert.match(sidecarProgramSource, /RestoreDatabase/)
  assert.match(sidecarProgramSource, /db\.Checkpoint\(\)/)
  assert.match(sidecarProgramSource, /CopyDatabaseAtomically/)
  ...
  assert.match(completenessSource, /packaged sidecar distribution remains a residual risk/)
})
