...
| local | LiteDB temporary file | local file | | | | | created by `npm run fixtures:validate:litedb`; optional real-engine file created by `fixtures:validate:litedb:dotnet` | local-file read/write open preflight, read-only write blocking, encryption posture, lock-boundary metadata, fixture-token dispatch, local sidecar-process evidence, and opt-in .NET sidecar collection/find/index validation |
