...
begin
  execute immediate 'drop table plan_table_xplan purge';
exception
  when others then
    if sqlcode != -942 then
      raise;
...
explain plan set statement_id = 'DATAPADPLUSPLUS'
into plan_table_xplan for
select *
from accounts
where id = 1;

select * from table(dbms_xplan.display('PLAN_TABLE_XPLAN', 'DATAPADPLUSPLUS', 'BASIC'));

drop table plan_table_xplan purge;
`)
