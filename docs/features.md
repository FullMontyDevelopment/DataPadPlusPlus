...
The Library connection tree and Explorer tabs let you browse a datastore before
querying it. Every supported engine supplies its own hierarchy, icons, counts,
inspection summaries, actions, and operational views through the same provider used
by both surfaces. Explorer uses independently scrolling navigation and detail panes,
search that preserves matching ancestors, bounded continuation paging, and explicit
loading, partial, empty, permission, connectivity, and retry states.

Explorer details are purpose-rendered as metrics, inventories, tables, health and
security summaries, or bounded type-aware values. Raw provider metadata is never
used as a user-facing fallback. System and administrative namespaces remain
available but are separated and collapsed by default. On narrow layouts the detail
surface becomes a dismissible right drawer. Explorer itself remains read-only;
querying and management open the datastore's existing guarded workflows.
...
SQL-family Explorer tabs use the datastore hierarchy by default and retain the
visual table and relationship map as a secondary view for understanding schemas,
columns, and joins before writing SQL.
...
Datastore Tests is an experimental, workspace-scoped plugin. It is disabled by default for new and existing workspaces; enable it from **Settings → Plugins** when a workspace needs repeatable datastore checks. Disabling the plugin hides its creation, Library, template, and search entry points without deleting saved suites, cases, results, drafts, or recoverable tabs.

Creating a suite opens a focused binding dialog. Choose one connection, one of that connection's environments, and a live Explorer target before the suite can be created. The target can be a database-level namespace or an eligible table, view, collection, Redis/Valkey prefix, or DynamoDB table. Explorer object actions open the same dialog with the object preselected. These bindings are immutable: create another suite when a check must run against a different connection, environment, or physical target.

A suite can include:

- one or more suite-owned Test Cases, each selectable beneath its parent suite in the Library
- setup, execute, and teardown phases
- query, visual builder, data-edit, and adapter-operation steps, with unsupported kinds blocked explicitly
...
- a visual-only case editor with add, duplicate, reorder, enable/disable, and remove actions
- removable steps and assertions, explicit assertion source-step selection, and per-step/case timeouts
- a visible connection, environment, target breadcrumb, target kind, and inferred query language shared by every owned case
- target-aware starter requests generated by the selected datastore provider

Query language is inferred from the bound connection and is read-only in the editor. Structured builders, edits, and operations must stay within the suite target. Raw requests remain available for compatibility, but preflight warns when an adapter cannot prove that a request is target-local or sees another object reference.

Run Suite executes enabled cases sequentially; Run Case scopes execution to the selected case. A preflight plan includes the immutable binding plus adapter capability, redacted generated requests, blockers, warnings, and any exact confirmation phrase required for writes. The target and inferred language are part of the plan fingerprint and persisted run summary. Supported query and builder steps execute through real datastore adapters—unsupported steps never fall back to synthetic passing results. Setup and Execute fail fast within a case, Teardown is attempted after failures, and ordinary case failures do not prevent later enabled cases from running. Browser preview supports configuration and visual editing but reports execution as desktop-only.

Suites are useful for validating fixture data, proving a saved query still returns the expected shape, and documenting operational checks beside the connection they rely on. Test execution follows the same read-only, safe-mode, environment, variable-resolution, and redaction posture as other workbench actions.
