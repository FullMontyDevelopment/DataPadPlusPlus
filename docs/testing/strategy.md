...
The separate Live Fixture Validation workflow starts PostgreSQL, MySQL, SQL Server, MongoDB, and Redis, runs the reference-engine validators and seven core Rust adapter tests, then executes the compiled desktop fixture journeys under Xvfb. It runs for relevant pull requests or manual dispatch; there are no scheduled runs. Each native build requires the .NET 10 SDK and preparation of the bundled Oracle runtime, even when the selected fixtures do not include an Oracle server. Oracle paging and selected-schema completion run only when manually selecting `oracle` or `all`, in an isolated job because Oracle Free has substantially higher startup and memory costs. Both jobs tear their containers down with unconditional cleanup steps.

Linux desktop fixture runs use `dbus-run-session` and `apps/desktop/e2e/with-linux-keyring.sh` to create an isolated, unlocked Secret Service. The encrypted fixture-secret file still requires an OS-vault master key; selecting the file store does not remove that dependency. The wrapper verifies credential write/read before starting the desktop and cleans up its temporary keyring afterward. Fixture startup aborts with an actionable error if credentials cannot be stored. Failed desktop runs retain native logs and screenshots for seven days; workspace files and secret-store files are excluded from artifact uploads.
